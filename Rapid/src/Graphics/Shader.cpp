...
GLuint Shader::getUniformDiffuse()
{
	return uniformDiffuse;
}

GLuint Shader::GetuniformSpecular()
{
...
GLuint Shader::GetFarPlanePosLocation()
{
...
void Shader::UpdateSpotLight(SpotLight* sLight, int index, float spotLightRedValue, float sapotLightGreenValue, float spotLightBlueValue, float spotLightIntensity, float spotLightDiffuseIntensity, float spotLightXDir, float spotLightYDir, float spotLightZDir)
{
	sLight->color.r = spotLightRedValue;
	sLight->color.g = sapotLightGreenValue;
	sLight->color.b = spotLightBlueValue;
	sLight->ambientIntensity = spotLightIntensity;
	sLight->diffuseIntensity = spotLightDiffuseIntensity;
	sLight->direction.x = spotLightXDir;
	sLight->direction.y = spotLightYDir;
	sLight->direction.z = spotLightZDir;
	sLight->UseLight(uniformSpotLight[index].uniformAmbientIntensity, uniformSpotLight[index].uniformColour, uniformSpotLight[index].uniformDiffuseIntensity,
		uniformSpotLight[index].uniformPosition, uniformSpotLight[index].uniformDirection, uniformSpotLight[index].uniformConstant, uniformSpotLight[index].uniformLinear,
		uniformSpotLight[index].uniformExponent, uniformSpotLight[index].uniformEdge);
}

void Shader::SetPointLights(PointLight* pLight, unsigned lightCount, unsigned int textureUnit, unsigned int offset)
{
...
void Shader::SetSpotLights(SpotLight* sLight, unsigned lightCount, unsigned int textureUnit, unsigned int offset)
{
	...
	for (size_t i = 0; i < lightCount; i++)
	{
		sLight[i].UseLight(uniformSpotLight[i].uniformAmbientIntensity, uniformSpotLight[i].uniformColour, uniformSpotLight[i].uniformDiffuseIntensity,
			uniformSpotLight[i].uniformPosition, uniformSpotLight[i].uniformDirection, uniformSpotLight[i].uniformConstant, uniformSpotLight[i].uniformLinear, uniformSpotLight[i].uniformExponent,
...
void Shader::CreateFromFiles(const char* vertexLocation, const char* geometryLocation, const char* fragmentLocation)
{
	std::string vertexString = Utils::ReadFile(vertexLocation);
	std::string fragmentString = Utils::ReadFile(fragmentLocation);
	std::string geometryString = Utils::ReadFile(geometryLocation);
...
void Shader::CompileProgram()
{
		...
		uniformDiffuse = glGetUniformLocation(shaderID, "material.diffuse");
